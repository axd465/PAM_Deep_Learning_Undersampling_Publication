...
def DownBlock(input, filters, kernel_size, padding, activation, kernel_initializer):
    ...
    out = Res_Block_Full_PreActivation(input, filters, kernel_size, strides=1, padding=padding, 
                                       activation=activation, kernel_initializer = kernel_initializer)
    shortcut = out
...
def UpBlock(input, filters, kernel_size, padding, activation, kernel_initializer):
    ...
    #print(filters)
    ############################################
    # Un-Modified UNet - Regular Up Block:
...
def ICL_Res_Block(input, filters, kernel_size=3, strides=1, padding='same', 
    ...
    residual = Conv2D_with_ICL(input, filters, kernel_size=1, strides=1, padding=padding, 
                               activation=activation, kernel_initializer = kernel_initializer, prob=0.05)
    out = Conv2D_with_ICL(input, filters, kernel_size, strides=1, padding=padding, 
                          activation=activation, kernel_initializer = kernel_initializer, prob=0.05)
...
def getModel(input_shape, filters, kernel_size, padding='same', 
    ...
    model_outputs = ResUNet(model_inputs, filters=filters, kernel_size=kernel_size, padding=padding, 
                            activation=activation, kernel_initializer=kernel_initializer)
    model = Model(model_inputs, model_outputs, name='ResUNet_Model')
