...
##################################################################################################################################
'''
STANDARDIZE DIR WITH INTERPOLATION UTILS:
'''
##################################################################################################################################
# Using RGB Channels to carry fully-sampled and downsampled grayscale data:
def pad_img_and_add_interp_down_channel(array=np.array(0), downsample_axis = 'x', downsample_ratio = [0,2], shape=[128,128]):
    '''
...
output = pad_img_and_add_interp_down_channel()

def standardize_dir_with_interp(input_dir = 'data/train/input', downsample_axis = 'x', downsample_ratio = [0,2], 
