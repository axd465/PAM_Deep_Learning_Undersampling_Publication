...
def UNet_model_simp_func(input, filters = 32, kernel_size = 3, padding = 'same', 
                         ...
                         strided_conv = False):
    shortcut1_1 = input
    [out, shortcut1_2] = DownBlock(input, filters, kernel_size, padding, activation, kernel_initializer, strided_conv)
    [out, shortcut2_1] = DownBlock(out, filters*2, kernel_size, padding, activation, kernel_initializer, strided_conv)
    [out, shortcut3_1] = DownBlock(out, filters*2*2, kernel_size, padding, activation, kernel_initializer, strided_conv)
    [out, shortcut4_1] = DownBlock(out, filters*2*2*2, kernel_size, padding, activation, kernel_initializer, strided_conv)
...
def DownBlock(input, filters, kernel_size, padding, activation, kernel_initializer, strided_conv):
...
def getModel(input_shape, filters, kernel_size, padding='same', activation='relu', 
             kernel_initializer='glorot_normal', strided_conv = False):
    ...
    model_outputs = UNet_model_simp_func(model_inputs, filters=filters, kernel_size=kernel_size, padding=padding, 
                                         activation=activation, kernel_initializer=kernel_initializer, strided_conv=strided_conv)
